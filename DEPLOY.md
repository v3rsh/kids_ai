...
| `DISK_WARN_PCT` | нет | Единственный порог предупреждения в чат модерации (без авто-действий), по умолчанию `45`. |
| `ARCHIVE_DISK_CAP_PCT` | нет | Потолок для архива полной базы (`bd-full.tar.gz`), по умолчанию `90`. Pre-flight отказывает, если после архивации диск заполнится на ≥ этого %. |
...
| `ARCHIVE_DIR` | нет | Каталог долговременной архивации копии `ATTACHMENTS_DIR` (по умолчанию `data/archive`). Pre-flight блокирует копию при `after_pct ≥ ARCHIVE_DISK_CAP_PCT`. |
...
- [ ] При искусственном заполнении `./data/attachments` свыше `DISK_WARN_PCT` (45 %) — в чат модерации приходит предупреждение, дедуп `1×24h` на порог; **автоматических действий с режимом приёма нет**.
- [ ] Переключение в `LINKS` выполняется только вручную админом (`/intake_mode` или `/admin_danger → force_links`).
- [ ] `/admin_purge_rejected_images` показывает статистику `99_rejected/` и после confirm удаляет только изображения (метаданные и `reason.txt` сохраняются).

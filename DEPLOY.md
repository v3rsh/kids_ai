...
| `ARCHIVE_DIR` | нет | Каталог долговременной архивации копии `ATTACHMENTS_DIR` (по умолчанию `data/archive`). Pre-flight блокирует копию при `after_pct ≥ DISK_BLOCK_PCT`. |
...
> чата модерации управляются только через discovery в боте (`/moderator`,
> `/jury`, кнопка «Сделать чатом модерации» при добавлении в групповой
> чат). Подробнее — `docs/deployment.md` → «Управление ролями через бот».
...
С HUID, добавленным через discovery (попроси кандидата написать `/moderator`,
админу в DM прилетит карточка — нажми «Назначить модератором»):

- [ ] `/queue` показывает поданную заявку с фильтрами по статусу/трекам.
- [ ] В чате модерации на новую заявку: блок «Быстрые команды», файлы, без кнопок; бот не реагирует на команды в группе. В личном DM: `/find BR-…` → карточка с фото.
- [ ] `/status BR-2026-NNNN модерация допущено` переводит заявку в `допущено` (кнопка «✅ Допустить» в карточке).
...
С HUID, добавленным через discovery (`/jury` от кандидата → карточка
админу → «Назначить членом жюри»):
...
| Уведомления модераторам не приходят | `/admin_state` → блок «Роли и каналы»: убедиться, что `Чат модерации валиден` + «бот в чате=да». Если нет — добавить бота в групповой чат и одобрить карточку через `/admin_chat_approve`. |
| Discovery-карточки админу не доходят | `/admin_state` → «DM-канал админа»: если не зафиксирован — нажать `/start` в личке бота. Без `users.chat_id` карточки молча no-op'аются. |
| Жюри не видит работ | Проверить, что назначение прошло (`/admin_roles`) и `JURY_POOLS_CONFIG` соответствует пулу; статус заявки должен быть `допущено`. |
| `/export` падает | Проверить, что в `requirements.txt` установлен `openpyxl` |

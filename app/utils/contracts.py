...
чтобы DTO были предельно лёгкими и хорошо хешировались (``PoolKey``
нужен ключом в словарях), без зависимости контрактов от pybotx/pydantic.
Если понадобится валидация — DTO можно обернуть в pydantic-модель локально,
не меняя контракт.
"""

...
MODERATOR_HELP_TEXT = (
    "**Команды модератора**\n\n"
    "**Очередь и разделы по статусам:**\n"
    "  /queue — очередь новых заявок (только на разборе)\n"
    "  /multi_subs — несколько заявок одного родителя в одном треке\n"
    "  /similar_apps — похожие работы (с карточки или из /multi_subs)\n"
    "  /queue_next — следующая заявка в очереди\n"
    "  /m_accepted — принятые заявки (трек → возраст → список)\n"
    "  /m_review — заявки на исправлении\n"
    "  /m_rejected — отклонённые заявки (без файлов)\n"
    "  /browse — карусель просмотра по фильтрам очереди\n"
    "  /find BR-2026-XXXX — карточка заявки (фото + действия)\n"

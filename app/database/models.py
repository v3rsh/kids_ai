...
class IntakeMode(enum.Enum):
    ...
    Переключение — только вручную админом (``/intake_mode`` или
    ``/admin_danger`` → ``force_links``). Автопереключения по диску нет.
    """
...
class DiskAlert(Base):
    ...
    Нужен для дедупликации: при достижении ``DISK_WARN_PCT`` (45 %)
    бот шлёт предупреждение в чат модерации не на каждой проверке
    (раз в 30 мин), а только при первом срабатывании порога — повторно
    не чаще раза в 24 ч. Записи старше 30 дней сервис ``storage`` может
    чистить вручную.
    """

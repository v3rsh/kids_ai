...
Замена SQLite-хранилища для высоконагруженных сценариев.
Полностью асинхронный, использует redis.asyncio с пулом соединений.
"""
import json
...
class RedisFSMStorage:
    ...
    async def cleanup_old_states(self, days: int = 30) -> int:
        ...
        поэтому эта операция — no-op. Метод сохранён для совместимости
        интерфейса с SQLite-хранилищем.

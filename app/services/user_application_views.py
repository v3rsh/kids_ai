...
заявок и ленту прогресса по этапам конкурса. Файлы работы
отображаются в хендлере ``user_applications`` (файлы работы и команда
``/my_app_files``). Служебные поля жюри и внутренние комментарии
модератора здесь не показываются.
"""

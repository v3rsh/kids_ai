"""
Сервис заявок ветки A (родитель/участник).

Реализует жизненный цикл модели ``Application``:
- генерация сквозного ``br_id`` формата ``BR-{COMPETITION_YEAR}-NNNN`` (§20);
- алгоритм автопометки «возможный дубль» (§15.3);
- создание заявки (§11, §14): атомарная транзакция с PostgreSQL
  advisory-lock'ом, чтобы две параллельные подачи не получили один br_id;
- маркировка актуальной версии заявки модератором (§15.2, поле №22).
...
async def assign_br_id() -> str:
    ...
    Используется в режиме ``links`` (§33.6.2), где br_id отдаётся
    участнику ДО запроса ссылки. В режиме ``files`` (основной) внутри
...
async def create_application(
    *,
    ...
    3. Поиск возможного дубля (§15.3) — отдельный SELECT по
       ``(parent_huid, track)`` с фильтрацией нормализованных имён
       в Python.
    4. INSERT в ``applications`` со ставкой ``moderation_status =
       НА_МОДЕРАЦИИ`` и проставленными полями №20/21 при наличии дубля.
    5. ``commit()`` — атомарно отпускает lock и фиксирует запись.

    Возрастная категория вычисляется автоматически из ``child_age``
    через ``AgeCategory.from_age`` (§8, §11.3). Невалидный возраст
    (вне 4..18) → ``ValueError``.
    """
    try:
        track_enum = Track[track_name]
...
async def mark_as_actual_version(
    *,
    ...
    Цепочка восстанавливается через ключи (§15.3): ``parent_huid``

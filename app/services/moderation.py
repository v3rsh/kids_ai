"""
Бизнес-логика ветки модератора (Wave 2 / B).

Сервис обслуживает команды модератора:

- ``list_queue`` — пагинация и фильтры (§27.1, §34.2);
- ``find_by_br_id`` — карточка заявки по ``BR-2026-XXXX`` (§27.1);
- ``change_status`` — смена статуса в одной из групп (§26, §27.1);
- ``add_comment`` — добавить комментарий модератора (§25.1 поле №15);
- ``count_stats`` — агрегированная статистика для ``/stats`` (§28).
...
- Возвращаемые типы — либо ORM-объекты ``Application`` (Wave 2 / B
  читает их в read-only из хендлеров и сразу формирует ответ), либо
  простые dataclass/dict для статистики.

Источник правды по полям статусов — ``database.models``:
``ModerationStatus``, ``JuryStatus``, ``VotingStatus``. Группа ``merch``
в БД отдельным enum не моделируется (§26 + Wave 0): «Статус мерча»
храним как свободный текст в ``Application.merch_potential``, в
Excel-витрине поле №19 «Потенциал для мерча» (§25.1) — то же самое.
"""
...
"""Группа статусов §26: ``moderation`` / ``jury`` / ``voting`` / ``merch``.

``jury`` — read-only для модератора (§25.3.3): ручное редактирование
запрещено, бот перезапишет. Сервис ``change_status`` для этой группы
вернёт ``ChangeStatusResult`` с ``ok=False`` и человекочитаемой ошибкой.
"""


@dataclass(frozen=True)
class QueueFilters:
    """Фильтры команды ``/queue`` (§34.2).
...
@dataclass(frozen=True)
class QueuePage:
    """Страница результата ``list_queue`` (§34.2).
...
@dataclass(frozen=True)
class StatsCounters:
    """Сводка статистики для ``/stats`` (§28).
...
# По §15 / §27 модератор работает с активной очередью —
...
def _moderation_status_by_value(value: str) -> ModerationStatus | None:
    """Сопоставить пользовательский ввод значению enum (§26)."""
...
async def list_queue(
    *,
    ...
    """Пагинация заявок для команды ``/queue`` (§34.2).
    ...
    Args:
        ...
        page: 1-индексированный номер страницы. Значения <1
            нормализуются к 1.
        page_size: размер страницы (по умолчанию 5, §34.2).
...
async def find_by_br_id(br_id: str) -> Application | None:
    """Карточка заявки по ``BR-2026-XXXX`` (§27.1 ``/find``).
...
async def change_status(
    *,
    ...
    """Сменить статус заявки в одной из 4 групп (§26, §27.1 ``/status``).

    Поведение по группам:

    - ``moderation`` — ``ModerationStatus`` (§26). Принимаются как
      ``.value`` («допущено»), так и ``.name`` (``DOPUSHCHENO``).
    - ``voting`` — ``VotingStatus`` (§26). Аналогично.
    - ``merch`` — пишет произвольный текст в ``Application.merch_potential``
      (§25.1 поле №19 «Потенциал для мерча»; в БД отдельной enum-группы
      нет — Wave 1 / Wave 0 решение).
    - ``jury`` — **запрещено** ручное редактирование (§25.3.3): сервис
    ...
    Args:
        ...
        by_huid: HUID модератора (для логов; в БД пока не пишется —
          журнал смен статусов будет в Wave 3, если попросит заказчик).
        ...
        if group == "jury":
            return ChangeStatusResult(
                ...
                error=(
                    ...
                    "по итогам процесса по пулу (§25.3.3). Ручное "
...
async def add_comment(
    *, br_id: str, text: str, by_huid: UUID
    ...
    """Добавить/перезаписать комментарий модератора (§25.1 поле №15).

    Текущая модель ``Application.moderator_comment`` — одно текстовое
    поле, поэтому предыдущий комментарий перезаписывается. Если в Wave 3
...
async def count_stats(period: StatsPeriod = "all") -> StatsCounters:
    """Агрегированная статистика по заявкам (§28, ``/stats``).

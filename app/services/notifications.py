"""
Сервис автосообщений конкурса «Безопасные рисунки» (ветка D3).

Содержит:
- module-level **шаблоны** сообщений (§18 — участнику, §19 — в чат модерации,
  §28.1 — alert'ы о диске). Шаблоны выделены, чтобы заказчик мог
  переопределить тексты через конфиг без правки кода (Wave 0 §36.1);
- функции отправки участникам (через ``bot.send_message`` с
  ``wait_callback=False``);
- функции отправки в чат «Безопасные рисунки — модерация» (UUID в
  ``MODERATION_CHAT_ID``);
- агрегатор событий жюри §19: одно сообщение со списком пулов на
...
# 1. Заказчик мог переопределить тексты через конфиг (Wave 0 §36.1).
...
"""§18.4 / §27.1 — опциональное уточнение модератора, добавляется к §18.4."""
...
"""§18.6.1 — финал жюри: вошла в топ-10."""
...
"""§18.6.2 — финал жюри: НЕ вошла в топ-10."""
...
"""§28.1 — alert 95 % (триггер автопереключения intake_mode)."""
...
"""§28.1 — сообщение участнику при попытке загрузить файл на 95 %."""
...
"""§33.6 — общее уведомление при переключении в режим LINKS."""
...
async def notify_participant_rejected(
    ...
    ``reason`` берётся из ``/notify_reject`` дословно (§27.1).
    """
    chat_id = await _resolve_user_chat_id(app.parent_huid)
    await _send_to_user(
        bot,
...
async def notify_participant_fix_needed(
    ...
    §27.1 — /notify_fix может передать ``текст_уточнения``; если передан,
    он добавляется к шаблону §18.4 через ``FIX_NEEDED_EXTRA_TEMPLATE``.
    """
...
async def notify_participant_jury_result(
    ...
    """§18.6 — итоговое сообщение по жюри."""
    body = (
        JURY_RESULT_IN_TOP10_TEMPLATE
        if in_top_10
...
# =====================================================================
# Сообщения в чат модерации (§19, §28.1)
# =====================================================================


def _format_files_pointer(app: "Application") -> str:
    """Поле «команда/ссылка просмотра файлов» (§19, §25.1 поле №13)."""
...
async def notify_moderation_chat_new_application(
    ...
    """§19 — служебное сообщение о новой заявке."""
...
async def notify_moderation_chat_disk_alert(
    ...
    *,
    ...
    """§28.1 — alert о заполнении диска (80 % / 95 %).
...
# =====================================================================
# Уведомления о событиях жюри (§19, §35.2, §35.5) — с агрегацией
# =====================================================================
#
# Правило агрегации (Wave 0 §19): открытие и закрытие раундов
# **агрегируются по моменту времени**. Если бот одновременно открывает
# или закрывает раунды сразу в нескольких пулах — отправляем одно
# сообщение со списком пулов.
#
...
async def notify_moderation_chat_jury_event(
    ...
    *,
    ...
    - ``lot_applied`` — индивидуально, без агрегации (Wave 0 §19).

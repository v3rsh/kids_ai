...
| `ADMIN_HUID` / `ADMIN_HUIDS` | UUID администраторов через запятую (тех. роль — `/disk`, `/intake_mode`; первый получает системные уведомления) |
| `MODERATOR_HUIDS` | UUID модераторов через запятую (§5.2). Синхронизируется в таблицу `moderators` на каждом старте |
| `JURY_HUIDS` | UUID членов жюри через запятую (§5.4). Синхронизируется в таблицу `jury_members` на каждом старте |
| `MODERATION_CHAT_ID` | UUID чата «Безопасные рисунки — модерация» (§19) для нотификаций о новых заявках и алёртов диска |

### Конкурс и реестр

| Переменная | Описание | По умолчанию |
|-----------|----------|-------------|
| `COMPETITION_YEAR` | Год конкурса — используется в `BR-{YEAR}-NNNN` и имени файла реестра (§4 `docs/registry-spec.md`) | `2026` |
| `JURY_POOLS_CONFIG` | JSON `[{"huid": "...", "pools": [...] \| "all"}]` (§35.6). Пустая строка = все судьи во всех 9 пулах | `""` |
| `INTAKE_MODE_DEFAULT` | `files` (основной) или `links` (резервный, §33.6) | `files` |
| `TOP_N` | Размер шорт-листа на пул (§35.1) | `10` |
| `JURY_ROUNDS` | Максимум раундов до жребия (§35.2) | `3` |
| `JURY_ROUND_DEADLINE_HOURS` | Дедлайн одного раунда (§35.6) | `48` |

### Хранилище и диск (§28.1)
...
| `MAX_FILE_SIZE_MB` | Лимит размера одного файла (§11.4, §16) | `10` |
| `DISK_WARN_PCT` | Порог предупреждения в чат модерации | `80` |
| `DISK_BLOCK_PCT` | Порог блокировки приёма + auto-switch в LINKS (§33.6, §28.1) | `95` |

...
| `MODERATOR_HUIDS` | UUID модераторов через запятую. Синхронизируется в таблицу `moderators` на каждом старте |
| `JURY_HUIDS` | UUID членов жюри через запятую. Синхронизируется в таблицу `jury_members` на каждом старте |
| `MODERATION_CHAT_ID` | UUID чата «Безопасные рисунки — модерация» для нотификаций о новых заявках и алёртов диска |

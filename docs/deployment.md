...
Бот **молча игнорирует** входящие во всех групповых чатах, кроме чата
модерации от пользователей с ролью модератора (см. `app/fsm/chat_gate.py`).
В уведомлении о новой заявке — текст `/find` / `/files`, кнопка
«📄 Карточка» и опционально deeplink «📄 Карточка в очереди»

...
Бот **молча игнорирует** входящие во всех групповых чатах, включая чат
модерации (см. `app/fsm/chat_gate.py`). В уведомлениях в чат модерации —
только текст (блок «Быстрые команды»: `/find`, `/files`); без инлайн-кнопок.
Модерация — в личном DM с ботом (`/moderator`, `/queue`, `/find`).

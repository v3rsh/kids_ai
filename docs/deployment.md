...
| `MODERATOR_HUIDS` | **Bootstrap only.** UUID модераторов через запятую. Загружается в таблицу `moderators` **только** при первом запуске (если таблица пуста). Дальше состав меняется через бот — discovery-кнопки админа + `/admin_roles`, `/admin_role_revoke`. Можно оставить пустым. |
| `JURY_HUIDS` | **Bootstrap only.** Аналогично `MODERATOR_HUIDS`, но для жюри. |
| `MODERATION_CHAT_ID` | **Bootstrap only.** UUID группового чата «Безопасные рисунки — модерация». Загружается в `app_settings.moderation_chat_id` **только** если настройки нет. Дальше — через `/admin_chat_approve` (карточка с кнопкой прилетает админу при добавлении бота в новый групповой чат). |
...
### Управление ролями через бот (после первого запуска)

После того как бот поднят с заполненным `ADMIN_HUID`, дальнейшая
конфигурация ролей и чата модерации делается **внутри бота**:

1. **Назначить модератора или жюри.** Попросите кандидата написать
   боту `/moderator` (или `/jury_menu`). Админу прилетит карточка
   ...
   - для модератора — попытка автодобавления в чат модерации
     (требует, чтобы бот был админом этого чата);
   - попытка отправить welcome-DM назначенному.
2. **Настроить чат модерации.** Создайте групповой чат и добавьте
   туда бота. На `ChatCreatedEvent` админу прилетит карточка с
   `chat_id` и кнопкой «Сделать чатом модерации». При нажатии бот
   запишет UUID в `app_settings.moderation_chat_id`, перечитает кэш
   и пошлёт в сам чат короткое подтверждение.
3. **Диагностика.** Команда `/admin_roles` (только в личном чате
   с админом) показывает все активные роли и текущий
   `moderation_chat_id`, с кнопками «🗑 Отозвать».

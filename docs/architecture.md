...
├── states.py            # FSM-состояния: UserIntake, ModeratorFlow / ModeratorAction,
│                        # JuryFlow / JuryTaskFlow, AdminFlow / AdminAction
...
│   ├── user_applications.py  # «Мои заявки» — список и карточки заявок родителя
...
│   ├── jury.py          # /jury, /jury_tasks (точка входа в роль)
...
│   ├── admin_users.py   # раздел «Пользователи»: find / resync / apps по HUID
│   └── admin_stats.py   # /admin_stats — расширенная аналитика
...
│   │                    # clear_moderation_chat (сброс при невалидном чате)
...
4. `reload_access_cache(session)` (`services.access`) — перечитывает
   таблицы и атомарно подменяет in-memory кэш ролей (`_moderator_huids`,
   `_jury_huids`, `_moderation_chat_id`). После этого `is_moderator()` /
   `is_jury()` / `get_moderation_chat_id()` отвечают O(1) без обращения
   в БД — критично для hot path (chat-gate middleware, `/moderator`,
   `/jury`). Env-seed модераторов / жюри / чата модерации **отключён**:
   единственный путь назначения — discovery (`/moderator`, `/jury`,
   кнопки админа в `handlers/admin_roles.py`).
...
8а. После входа в `lifespan_wrapper(bot)` — `_validate_moderation_chat(bot)`
    ...
    дёргаем `bot.chat_info`. Любая ошибка (бот не участник, чат удалён,
    UUID-болванка) → `clear_moderation_chat()` сбрасывает настройку,
    в логе ERROR с причиной. Это спасает от тихого дропа уведомлений
    в несуществующий чат при `wait_callback=False`.
...
Env-seed (`MODERATOR_HUIDS` / `JURY_HUIDS`) **отключён** — состав
меняется только кнопками админа (discovery-карточки на `/moderator` и
`/jury`, диагностика через `/admin_roles`, отзыв через
...
Заведены семь классов состояний — анкета участника, контекст ветки
модератора (меню + диалоговые подсказки), контекст ветки жюри
(меню + прохождение задачи) и админ-меню:
...
| `AdminFlow` | admin_menu | Главное меню админки (после `/admin`). Свободный текст перерисовывает меню админа |
| `AdminAction` | add_role_role, add_role_huid, find_user_huid, chat_test_text, shortcut_find_brid, resend_welcome_* | Диалоговые шаги админки: ввод HUID, текста тестового сообщения, BR-ID |
...
UUID чата модерации — в `app_settings.moderation_chat_id`. Env-seed
этих сущностей **отключён** — переменных `MODERATOR_HUIDS` / `JURY_HUIDS`
/ `MODERATION_CHAT_ID` в проекте больше нет. Управление полностью через
discovery: команды `/moderator` и `/jury` отправляют админу карточку с
профилем кандидата и кнопками «Назначить / Отклонить»; при добавлении
бота в групповой чат админу приходит карточка «Сделать чатом модерации?».
Диагностика — `/admin_roles` и `/admin_state`, отзыв — `/admin_role_revoke`.
...
Когда юзер без роли дёргает `/moderator` или `/jury`, а также
...
- `discovery.send_welcome_dm_to_moderator(bot, huid)` шлёт
  приветственное DM **с прикреплённым меню роли** (`/queue`,
  `/browse`, …, «Назад в главное меню») и одновременно записывает
  в Redis FSM-state `moderator:menu` (для жюри — аналогично
  `jury:menu` через `send_welcome_dm_to_jury`). Так модератор/судья
  оказывается «в меню роли» сразу после одобрения, без необходимости
  вручную вводить `/moderator` или `/jury`; команды роли работают
  с первого же клика. Если у пользователя нет `users.chat_id`
  (никогда не писал боту `/start`) — DM не отправляется, но
  FSM-state всё равно записывается: при первом же входе из главного
  меню или прямом вызове `/moderator`/`/jury` контекст уже корректен.
...
Конструкторы меню роли (`moderator_menu_bubbles` / `jury_menu_bubbles`)
живут в `app/keyboards.py`, а не в хендлерах: тот же набор кнопок
переиспользуется и в welcome-DM из `services/discovery.py`, и в
точках входа `cmd_moderator_menu` / `cmd_jury_menu`. Возврат в главное
меню — кнопка `/start` под меню роли: `handlers/common.py::cmd_start`
снабжён `fsm_middleware` и в самом начале вызывает `fsm.clear()`,
гарантируя выход из ветки роли.

### Админ-меню (`/admin`)

Точка входа — скрытая команда `/admin` (`visible=False`, только
`ADMIN_HUIDS` из env; ввод вручную).
Кнопка в главном меню участника **не** добавляется: админ открывает
меню явной командой.

Структура зеркалит модераторское меню: главный экран → разделы →
действия → «◀ В админку». Бейджи на кнопках главного меню собираются
одной выборкой через `services.admin.overview_counters()`.
...
не поддерживает `edit_message(file=...)` для замены фото. При
``total > 1`` стрелки prev/next **зациклены**; кнопка «Следующая без
оценки» ищет следующую работу без черновика по кругу. Повторный
``/jt_open`` стартует на первой неоценённой (если все оценены — на
работе 1). Видимый

# Тестирование kids_ai

> Этот документ — единственный источник правды по тестированию проекта:
> что и **как запускать**, что **покрыто** автотестами, и какой
> минимальный сценарий нужно прогнать вручную перед деплоем (см. §6 ниже).
> Исторический контекст оригинального чек-листа — в архиве
> [`docs/history/ТЗ.md`](history/ТЗ.md), к нему не возвращаемся.

---

## 1. Установка зависимостей

```bash
pip install -r requirements.txt
```

Тестовый блок в `requirements.txt`:

```
pytest>=8.0.0
pytest-asyncio>=0.23.0
```

> ⚠️ Если ставите в системный Python (без venv) и видите конфликт
> с `aiogram` / `pydantic>=2` — поднимите venv: тесты не зависят от
> aiogram, но pip ругается. Pytest при этом запускается нормально.

---

## 2. Запуск всех тестов

Из корня репозитория:

```bash
python -m pytest tests/ -q
```

`asyncio_mode=auto` (см. `pytest.ini`) — все `async def`-тесты
запускаются без явного маркера `@pytest.mark.asyncio`.

Параметры:

- `-x` — остановиться на первой ошибке (CI/быстрый локальный smoke);
- `-k <pattern>` — выборочный запуск по имени;
- `--lf` — перепрогон только упавших с прошлого запуска.

### Пример: только flow-тесты

```bash
python -m pytest tests/test_application_flow.py tests/test_jury_flow.py -q
```

---

## 3. Структура тестов

| Файл | Что покрывает | Связанные документы |
|------|---------------|---------------------|
| `tests/conftest.py` | Общий setup: env (`BOT_ID`/`CTS_URL`/...), `sys.path` для `from services import ...`, маркер `slow` | — |
| `tests/test_validation.py` | Валидаторы и нормализаторы пользовательского ввода (анкета участника). **1 пре-existing fail в `test_sanitize_input`** — оставлен под отдельную правку | [`architecture.md`](architecture.md) → «FSM-система» |
| `tests/test_jury_algorithm.py` | 3 классических кейса алгоритма раунда: 1 раунд без ничьи, ничья на границе, эскалация в раунд 3 | [`architecture.md`](architecture.md) → «Модель данных» (jury_rounds, jury_votes) |
| `tests/test_application_flow.py` | `services.applications`: normalize_child_name, child_submission_key, `_group_applications`, find_possible_duplicate, валидация `intake_mode` | [`architecture.md`](architecture.md) → «applications» |
| `tests/test_moderation_flow.py` | `services.moderation`: parse_status_group алиасы, _moderation_status_by_value / _voting_status_by_value, _build_queue_where_clauses, DEFAULT_QUEUE_STATUSES | [`architecture.md`](architecture.md) → «applications» (поля статусов) |
| `tests/test_deeplink.py` | `utils.deeplink`: `build_bot_deeplink`, `build_find_deeplink` (плейсхолдеры, encoding, graceful degradation) | [`architecture.md`](architecture.md) → «Deeplink в чате модерации» |
| `tests/test_jury_flow.py` | Расширение `test_jury_algorithm`: размер top_n, above_tie == TOP_N, детерминизм сортировки, `services.pools.all_pools()` = 3×3 = 9 пулов | [`architecture.md`](architecture.md) → «jury_pool_assignments» |
| `tests/test_registry.py` | `services.registry`: `registry_export_filename`, `transliterate_icao_9303`, `jury_column_header`, `view_command_or_link`, `contact_field`, `jury_outcome` + smoke-рендер XLSX через `_render_registry_workbook` (без БД) | [`registry-spec.md`](registry-spec.md) → §2.2, §2.3.1, §4 |
| `tests/test_discovery_welcome.py` | `services.discovery`: welcome-DM модератору и жюри несёт меню роли (`/queue`, `/jury_tasks`, …) и параллельно ставит FSM-state `moderator:menu` / `jury:menu`; крайний кейс — нет `users.chat_id` (DM не уходит, но state записывается) | [`architecture.md`](architecture.md) → «Discovery» |
| `tests/test_keyboards.py` | `keyboards`: `back_to_*_menu_bubbles`, `fix_needed_notification_bubbles` — команды кнопок возврата | [`architecture.md`](architecture.md) → «Навигация в одном сообщении» |
| `tests/test_notifications.py` | `services.notifications`: проактивные DM участнику (`notify_participant_*`) всегда передают `bubbles` (`/start`, для fix — ещё `/menu_contacts`) | [`architecture.md`](architecture.md) → «Навигация в одном сообщении» |
| `tests/test_intake_state.py` | `services.intake_state`: дефолт «открыт» при пустой записи, persistence/idempotency, парсинг truthy/falsy/мусора | [`architecture.md`](architecture.md) → «Закрытие приёма заявок» |
| `tests/test_attachments_export.py` | `services.attachments_export`: meta.txt, manifest CSV (UTF-8+BOM, `;`), links.txt, сборка entries для FILES/LINKS-ссылка/LINKS-черновик/oversize, группировка пула в tar.gz и split на части, пустой селектор | [`architecture.md`](architecture.md) → «Архивная выгрузка `data/attachments`» |
| `tests/test_attachments_archive.py` | `services.attachments_archive`: pre-flight `ArchiveBudgetExceeded`, сборка `bd-full.tar.gz` с manifest+summary (внутри tar и рядом), ротация `bd-full.prev.tar.gz`, пустой источник | [`architecture.md`](architecture.md) → «Архивная выгрузка `data/attachments`» |

---

## 4. Архитектурные принципы тестов

1. **Без поднятия PostgreSQL.** Модели используют PG-специфичные
   фичи (`UUID(as_uuid=True)`, `JSONB`, `pg_advisory_xact_lock`),
   поэтому полная интеграция остаётся в **ручном smoke-чек-листе**
   (см. §6). Юнит-тесты гоняем на чистых функциях и мок-сессиях.
2. **Без поднятия Redis.** FSM-storage в тестах не нужен — мы тестируем
   слой сервисов, а не handler'ы pybotx.
3. **Pure functions сначала.** Где можно — тестируется чистая функция
   (например, `normalize_child_name`, `_compute_outcome_from_data`,
   `transliterate_icao_9303`). Это даёт быстрый и стабильный сигнал
   на регресс алгоритмов.
4. **Async через `AsyncMock`.** Где функция требует session — отдаём ей
   мок `MagicMock`/`AsyncMock` с предсказанным `execute` →
   `scalar_one_or_none`. Цель — проверить ветвление логики, а не SQL.
5. **XLSX без записи на диск.** `_render_registry_workbook` пишет в
   `BytesIO`; тесты проверяют сигнатуру `b"PK"` (ZIP-magic) и
   количество колонок/строк. Это smoke на «реестр собирается».

---

## 5. Что НЕ покрыто автотестами и почему

- **Полный pybotx-flow** (нажатие кнопок, edit_message, transient
  cleanup) — требует поднятого pybotx-тест-стенда. Идёт по ручному
  чек-листу §6.
- **PostgreSQL advisory_xact_lock + конкурентный `assign_br_id`** —
  требует реального PostgreSQL, идёт по ручному чек-листу + ревью.
- **Реальная отправка XLSX в чат** — pybotx `bot.answer_message` mock'ом
  не покрывается, проверяется ручным `/export` / `/export_shortlist`.
- **Жюри-flow «открытие раунда → голосование → закрытие → жребий»**
  end-to-end — модели жюри связаны с PostgreSQL JSONB-снимками. Тесты
  алгоритма (`_compute_outcome_from_data`) покрывают всю математику;
  координацию проверяет ручной сценарий.

---

## 6. Ручной приёмочный чек-лист

Минимальный сценарий перед каждым релизом. Полный набор покрывается
обычной приёмкой; здесь — обязательный smoke по всем веткам бота.

**Участник:**
1. `/start` в личке → главное меню → «Подать заявку» → выбор трека.
2. Пройти анкету до шага «Файлы» — загрузить 1 файл и затем серию
   2–4 файлов (для треков с несколькими ракурсами), **каждый следующий
   только после кнопки «Добавить ещё файл»**. Отправить 2 файла подряд
   без кнопки — ожидаем ошибку «за одну загрузку принимается один файл».
   На треках «ИИ-рисунок» и «От руки к ИИ» второй файл в любой момент —
   ошибка «ровно один файл».
   Прикрепить 2+ файла к **одному** сообщению на шаге «Файлы» — бот
   полностью отвергает сообщение (transient), ни один файл не сохраняется,
   выдаёт понятный текст про «один файл за одну отправку».
   При приёме каждого файла бот **отдаёт этот же файл обратно**
   persistent-сообщением с подписью «Шаг 7 из 7. Принят файл «…» …».
   Сообщение остаётся в истории; пользователь видит имя принятого файла
   из пачки.
3. Попробовать PDF/HEIC, отказать MP4 (неподдерживаемый формат) и
   файл > `MAX_FILE_SIZE_MB` (по умолчанию 10 МБ).
4. Поставить согласия → подтвердить → получить сообщение «Заявка
   принята и передана на модерацию» с присвоенным `BR-2026-NNNN`.
5. Повторно подать заявку на того же ребёнка и трек → автопометка
   «возможный дубль» и блок «Повтор по правилу 1 работа в трек» в карточке.
6. `/multi_subs` (модератор) — список групп; «Актуальная» / `/find` по BR-ID.
7. После `/notify_fix` — «Подать исправленную работу» (`/apply_fix`) → тот же
   BR-ID, статус снова «на модерации».
8. «Мои заявки» → список с номером и статусом → открыть карточку
   (превью работы + текст подписью, кнопка «Все файлы»);
   после `/notify_fix` / `/notify_reject` — проверить текст модератора
   и кнопки (исправление → «Подать исправленную работу»).

**Модератор:**
5. В чате модерации на новую заявку: текст «Быстрые команды», файлы, **без**
   кнопок; бот не отвечает на `/find` в группе. В личном DM: `/find BR-…` →
   карточка с файлами (первый — полная информация + кнопки, остальные — «файл N из M»).
6. `/queue` (дефолт `на_модерации + нужно_исправить`), затем `/queue all`.
7. `/find BR-2026-0001` → карточка с инлайн-кнопками и автоотправкой всех файлов.
8. `/status BR-... модерация допущено`, `/comment BR-... <текст>`.
9. `/notify_fix BR-... <причина>`, `/notify_reject BR-... <причина>` —
   проверяем доставку шаблонных сообщений участнику.
10. `/files BR-...` в режиме `files` → файлы приходят в чат
    attachment'ами.
11. `/export` → XLSX `registry_BR-2026_YYYY-MM-DD_HH-MM.xlsx`
    (см. [`registry-spec.md`](registry-spec.md) §4).
12. `/export_shortlist` → XLSX `shortlist_BR-...`.
13. `/stats today`, `/stats all`.

**Жюри:**
14. `/jury_tasks` → карусель задач, голос «Да»/«Нет» на каждой.
15. Кнопка «Отправить оценки» неактивна до полного покрытия + наличия
    обоих типов голосов (правило разброса — есть и YES, и NO).
16. Закрыть пул досрочно `/jury_close_round <pool>`, проверить
    обновление колонок 23–29 в `/export`.
17. Принудительно вызвать жребий через искусственный 3-й раунд с ничьёй.

**Жюри: edit-in-place карусель (mobile-QA, обязателен на iOS и Android):**

> Гибридная схема «photo-якорь + edit caption» — см.
> [`docs/architecture.md` §6](architecture.md). Цель проверок ниже —
> убедиться, что **viewport не уезжает на голосовании**, а старые якоря
> не остаются «сиротами» после выхода.

18. **Голос на 1-файловой работе.** Открыть задачу с одним файлом,
    нажать «Да» → caption и эмодзи кнопок обновляются на месте, **фото
    не двигается, чат не прокручивается вверх**. Затем «Нет» → то же.
19. **Голос на 3-файловой работе.** Открыть задачу с 3 файлами;
    проверить, что под якорем видны 2 transient-файла («Файл 2 из 3»,
    «Файл 3 из 3»). Нажать «Да» → caption и кнопки якоря меняются,
    **2 transient-файла остаются на месте**, чат не прыгает.
20. **Навигация prev/next** (ожидаемо «прыжок» — это не баг).
    Из работы с 3 файлами нажать «Следующая» → старая карточка с
    хвостом исчезает, новая приходит снизу. На **последней** работе
    «Следующая →» ведёт на **первую**; на **первой** «← Предыдущая» —
    на **последнюю**. На работе с 0 файлами
    (intake LINKS) проверить text-якорь без вложений.
20a. **«Следующая без оценки».** При частичных черновиках кнопка
    видна; после оценки всех работ исчезает. Клик при полном покрытии
    (устаревшая кнопка) — остаётесь на той же работе, transient
    «Все работы в этом раунде уже оценены».
20b. **Resume при входе.** Выйти в меню, снова открыть тот же раунд
    с частичными черновиками — старт **не** на работе 1, а на первой
    без оценки; если все оценены, но «Отправить» не нажато — работа 1.
21. **Выход через «📋 В меню задач».** Старая карточка-якорь должна
    **полностью исчезнуть** из чата (не оставаться с устаревшими
    кнопками); список задач приходит как новое сообщение.
22. **Выход через «✓ Отправить оценки».** После успеха якорь удалён,
    приходит «✅ Оценки отправлены», затем — список задач без
    отправленного раунда.
23. **Раунд закрыт во время работы.** Админ закрывает пул; следующий
    клик «Да/Нет/Следующая» → судья получает «Раунд закрыт…», старый
    якорь исчезает. (Если хвост был — `cleanup_middleware` снимет.)
24. **Перезаход в карусель.** `/jury_tasks` → открыть задачу → выйти
    «📋 К списку» → открыть **другой** раунд: старого якоря быть
    не должно (исчез на шаге выхода), новый раунд отрисован чисто.
25. **Двойной клик «Да → Следующая».** Голос edit'ом, потом nav delete+send;
    в чате должно остаться: новая карточка снизу, без дубликатов
    старой и без артефактов в FSM (`jury_task_anchor_sync_id` =
    sync_id новой карточки).
26. **Зацикленная навигация.** На первой работе «← Предыдущая» →
    последняя; на последней «Следующая →» → первая (обе стрелки
    видны при `total > 1`).
27. **Resume при повторном входе.** Оценить 1-ю работу, выйти в меню,
    снова открыть тот же раунд → старт на первой **неоценённой**; если
    все оценены — на работе 1.
28. **«Следующая без оценки».** При нескольких неоценённых — переход
    по кругу; когда осталась только текущая — кнопки нет; при гонке
    (оценили все, кнопка ещё видна) — остаёмся на месте, transient
    «Все работы в этом раунде уже оценены.».

**Админ:**
18. `/disk` — занятость диска + строка «приём блокируется при ≥ 95%».
19. `/intake_mode links` → переключение в резервный режим. Сразу
    после этого:
    - под чужим аккаунтом начать новую анкету, дойти до резюме;
    - убедиться, что в резюме строка «**Работа:** будет загружена
      ссылкой на облачную папку…» вместо «Файлы: N»;
    - нажать «Отправить заявку» → бот выдаёт `BR-2026-NNNN` и
      инструкцию по §33.6.2 (имя папки, шаблон файлов по треку);
    - отправить файл вместо текста → transient «нужна ссылка, не файл»;
    - отправить мусорный текст → transient про https/http;
    - прислать валидную ссылку (https://…/) → бот отвечает «Заявка
      принята… Номер заявки … Ссылка …», в чате модерации появляется
      карточка с `🔗 Ссылка на папку:`.
    - В «Мои заявки» открыть карточку — статус «Ожидает ссылку», есть
      кнопка «🔗 Прислать ссылку на папку» (recovery после рестарта
      бота / Redis). Нажатие восстанавливает шаг сбора URL.
    - Вернуть режим: `/intake_mode files`.
20. `/admin` → главное меню админки с бейджами (роли, чат, диск).
21. Раздел «Роли»: `/admin_role_add` (FSM HUID), отзыв с подтверждением,
    `/admin_role_resend_welcome`.
22. Раздел «Чат модерации»: `/admin_chat_status`, тестовое сообщение,
    `/admin_chat_rediscover`.
23. Раздел «Пользователи»: `/admin_user_find <huid>` → карточка, resync CTS.
24. `/admin_stats` → расширенная аналитика; опасные операции — двойное
    подтверждение через `/admin_danger` → `/admin_confirm`.
25. `/admin_state` → диагностика FSM, ролей и пулов.

---

## 7. Расширение покрытия

- Новые модули `tests/test_*.py` — следовать паттерну из
  `tests/conftest.py` (env + `sys.path`).
- Для async-тестов **не** добавлять `@pytest.mark.asyncio` вручную:
  `asyncio_mode=auto` сделает это за тебя.
- Маркер `@pytest.mark.slow` зарезервирован для медленных тестов
  (рендер большого XLSX, генерация превью с реальной PIL и т. п.) —
  при появлении конкретных слоёв задавай явно, чтобы CI мог
  селективно запускать `python -m pytest -m "not slow"` для быстрого
  обратного цикла.

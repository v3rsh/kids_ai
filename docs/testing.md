...
2. Пройти анкету до шага «Файлы» — загрузить 1 файл и затем серию
   2–4 файлов (для треков с несколькими ракурсами), **каждый следующий
   только после кнопки «Добавить ещё файл»**. Отправить 2 файла подряд
   без кнопки — ожидаем ошибку «за одну загрузку принимается один файл».
   На треках «ИИ-рисунок» и «От руки к ИИ» второй файл в любой момент —
   ошибка «ровно один файл».
   Прикрепить 2+ файла к **одному** сообщению на шаге «Файлы» — бот
   полностью отвергает сообщение (transient), ни один файл не сохраняется,
   выдаёт понятный текст про «один файл за одну отправку».
   При приёме каждого файла бот **отдаёт этот же файл обратно**
   persistent-сообщением с подписью «Шаг 7 из 7. Принят файл «…» …».
   Сообщение остаётся в истории; пользователь видит имя принятого файла
   из пачки.
...
**Модератор:**
5. В чате модерации на новую заявку: текст «Быстрые команды», файлы, **без**
   кнопок; бот не отвечает на `/find` в группе. В личном DM: `/find BR-…` →
   карточка с файлами (первый — полная информация + кнопки, остальные — «файл N из M»).
6. `/queue` (дефолт `на_модерации + нужно_исправить`), затем `/queue all`.
7. `/find BR-2026-0001` → карточка с инлайн-кнопками и автоотправкой всех файлов.
...
19. `/intake_mode links` → переключение в резервный режим. Сразу
    после этого:
    - под чужим аккаунтом начать новую анкету, дойти до резюме;
    - убедиться, что в резюме строка «**Работа:** будет загружена
      ссылкой на облачную папку…» вместо «Файлы: N»;
    - нажать «Отправить заявку» → бот выдаёт `BR-2026-NNNN` и
      инструкцию по §33.6.2 (имя папки, шаблон файлов по треку);
    - отправить файл вместо текста → transient «нужна ссылка, не файл»;
    - отправить мусорный текст → transient про https/http;
    - прислать валидную ссылку (https://…/) → бот отвечает «Заявка
      принята… Номер заявки … Ссылка …», в чате модерации появляется
      карточка с `🔗 Ссылка на папку:`.
    - В «Мои заявки» открыть карточку — статус «Ожидает ссылку», есть
      кнопка «🔗 Прислать ссылку на папку» (recovery после рестарта
      бота / Redis). Нажатие восстанавливает шаг сбора URL.
    - Вернуть режим: `/intake_mode files`.
20. `/admin` → главное меню админки с бейджами (роли, чат, диск).
21. Раздел «Роли»: `/admin_role_add` (FSM HUID), отзыв с подтверждением,
    `/admin_role_resend_welcome`.
22. Раздел «Чат модерации»: `/admin_chat_status`, тестовое сообщение,
    `/admin_chat_rediscover`.
23. Раздел «Пользователи»: `/admin_user_find <huid>` → карточка, resync CTS.
24. `/admin_stats` → расширенная аналитика; опасные операции — двойное
    подтверждение через `/admin_danger` → `/admin_confirm`.
25. `/admin_state` → диагностика FSM, ролей и пулов.

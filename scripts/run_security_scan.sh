...
# Скрипт для запуска SAST-анализа с использованием Bandit
...
HTML_REPORT="${REPORTS_DIR}/security_report_${TIMESTAMP}.html"

echo -e "${YELLOW}Запуск сканирования безопасности кода...${NC}"

# Запускаем Python-скрипт для анализа
python3 "${SCRIPT_DIR}/run_security_scan.py" --html --output "security_report_${TIMESTAMP}.html"
